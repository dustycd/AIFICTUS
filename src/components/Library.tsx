...
const Library = () => {
  ...
  const generateAIOrNotData = (item: LibraryItem) => {
    ...
    // Simulate AI or Not API response structure
    const aiConfidence = Math.random() * 0.4 + 0.1; // 10-50% AI confidence
    const humanConfidence = 1 - aiConfidence;
    
    const isAI = aiConfidence > 0.3;
    const finalAiProb = isAI ? (aiConfidence * 100) + Math.random() * 20 : Math.random() * 30;
    const finalHumanProb = 100 - finalAiProb;
    
    // Generate detection details based on AI or Not API structure
    const detectionDetails = {
      faceAnalysis: 75 + Math.random() * 20,
      compressionArtifacts: 80 + Math.random() * 15,
      ...(isVideo && {
        temporalConsistency: 70 + Math.random() * 25,
        audioAnalysis: 65 + Math.random() * 30,
      }),
      ...(!isVideo && {
        metadataAnalysis: 85 + Math.random() * 10,
        pixelAnalysis: 78 + Math.random() * 18,
      }),
    };
    ...
    return {
      ...
      verification_status: isAI ? 'fake' : (finalHumanProb > 80 ? 'authentic' : 'suspicious'),
      processing_time: 1.5 + Math.random() * 8,
      detection_details: detectionDetails,
      risk_factors: riskFactors,
      recommendations: recommendations,
      report_id: `rpt_${Math.random().toString(36).substr(2, 9)}`,
      api_verdict: isAI ? 'ai' : 'human',
      generator_analysis: {
        [topGenerator]: generatorConfidence,
      ...
      raw_api_response: {
        ...
        report: {
          ...
          human: { confidence: humanConfidence },
          generator: { [topGenerator]: generatorConfidence }
        },
        facets: facets
      }
    };
