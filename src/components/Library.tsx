        ...
        return { 
          ...
          icon: AlertTriangle,
                  {filteredItems.filter(item => {
                    const display = getVerificationDisplay(item.ai_probability, item.human_probability, item.verification_status);
                    return display.status === 'authentic';
                  }).length}
         ...
         <div className="bg-gray-900 rounded-2xl border border-gray-700 max-w-5xl w-full max-h-[90vh] overflow-y-auto"
          ...
          <div ref={reportRef} className="p-8 pt-24 bg-gray-900">
            ...
            <div className="space-y-8">
                  ...
                  return mediaUrl ? (
                    <>
                      {isVideo(selectedItem.content_type) ? (
                        ...
                        <img
                      <div className={`absolute top-3 left-3 px-3 py-1 rounded-full border backdrop-blur-sm ${getStatusBadgeClasses(display.status)}`}>
                          alt="Media content"
                          {display.status === 'authentic' ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <div className="text-center">
                        {isVideo(selectedItem.content_type) ? (
                          ...
                          {formatConfidence(display.confidence)}
                        )}
                        <Typography variant="body" color="secondary">
              ...
              {(() => {
                const display = getVerificationDisplay(selectedItem.ai_probability, selectedItem.human_probability, selectedItem.verification_status);
                ...
                return (
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-2">
                      ...
                      <div className={`bg-gradient-to-br ${display.bgColor} rounded-2xl p-6 border ${display.borderColor} text-center h-full flex flex-col justify-center`}>
                        <div className={`flex justify-center mb-4 ${display.statusColor}`}>
                          {display.status === 'authentic' ? (
                        ...
                        <Typography variant="h3" className={`mb-2 uppercase ${display.statusColor}`}>
                          {display.displayStatus}
                        </Typography>
                        <Typography variant="h2" className={`mb-2 numeric-text ${display.statusColor}`}>
                          {formatConfidence(display.confidence)}
                        </Typography>
                  <div className="space-y-3 text-center">
                    ...
                    <div className="pt-2 border-t border-gray-700">
                          {selectedItem.ai_probability !== undefined && (
                            <div className="flex justify-between">
                              <span className="text-gray-400">AI Probability</span>
                              <span className="numeric-text text-red-400">
                                {selectedItem.ai_probability.toFixed(1)}%
                              </span>
                            </div>
                          )}
                          
                          {selectedItem.human_probability !== undefined && (
                            <div className="flex justify-between">
                              <span className="text-gray-400">Human Probability</span>
                              <span className="numeric-text text-green-400">
                                {selectedItem.human_probability.toFixed(1)}%
                              </span>
                            </div>
                          )}
...
export default Library;
                <option value="authentic">Human Created</option>
                <option value="fake">AI Generated</option>